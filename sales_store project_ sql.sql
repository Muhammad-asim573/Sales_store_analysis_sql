...
--Optimize Fulfillment Speeds to Reduce Cancellations in Books: Customers are cancelling book orders before they 
--even receive them (26.20% cancellation rate is very high). This suggests shipping estimates are either too long
--or inaccurate.
...
--Identify and Fix Root Causes of High Returns in Accessories
-- we can launch a specific investigation or customer survey for Accessories. If the issue is poor
--product descriptions or misleading photos, fixing this on the website can decrease the return rate 
--from 31.5% to a lower baseline, directly increasing profit margins for that category without needing 
...
--Business Problem:don't know which payment option customers prefer.
...
SELECT TO_CHAR(purchase_date,'YYYY-MM')AS monthly_basis,
      '₹' || TO_CHAR(SUM(price*quantity), 'FM99,99,99,999.00')AS total_sales,
	   ...
	   ORDER BY monthly_basis;

--Business Problem:sales fluctuation go unnoticed.

--Business Impact:now we know that which month are making how much so according to that we can plan inventory ,
--making marketing strategy according to seasonal trends.
--Staff Planning:By knowing when we busy and slow periods are, we can effectively manage staffing levels,  	   
--Business Insight:The data reveals that our business operates on a highly predictable seasonal cycle.
--Sales fluctuate significantly throughout the year, with specific periods of high demand and low demand. 
--The difference between our lowest month (Month 5 at ₹39L) and our highest month (Month 10 at ₹58L) is nearly ₹20 Lakh, 
--which is a massive opportunity for optimization.
...
SELECT product_category,
    SUM(CASE WHEN gender='M'THEN quantity ELSE 0 END )AS male_unit_sold,
	SUM(CASE WHEN gender='F'THEN quantity ELSE 0 END )AS female_unit_sold,
	SUM(quantity)as total_quantity
FROM sale
GROUP BY product_category
ORDER BY product_category;

--Business Problem: Ineffective Inventory Management and Misaligned Marketing for Specific Categories

--Business Impact:personized ads,gender-focused campaigns.
--Specific Observations from the Data:
--Clothing: Females buy significantly more clothing items (581 vs. 518 units), indicating a strong female preference.
--Furniture & Groceries: Slight female preference.
--Electronics, Accessories, & Books: Slight male preference.
